...
Parser.prototype.write = function (data1) {
  // if (!data || !Buffer.isBuffer(data)) { throw new TypeError('data (buffer) required') }
  if (!data1) { throw new TypeError('data (buffer) required') }

  let nextMessage = null
  const self = this
  const data = self.buffer ? Buffer.concat([self.buffer, data1]) : Buffer.from(data1)
